...
useless_features = ['Date', 'Promo2SinceWeek', 'PromoInterval', 'CompetitionOpenSinceYear', 'CompetitionOpenSinceMonth',
                    'Promo2SinceYear']
...
if sample:
    if not gridsearch:
        ...
        if logexp:
            print "RMSPE: " + str(
                rmspe(map(lambda x: np.exp(x) - 1, regressor.predict(np.array(test[features]))), test[goal].values))
        else:
