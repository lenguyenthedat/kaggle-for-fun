...
def SGD(train,test,features):
    # Training / Cross Validation
    if sample:
        cv = cross_validation.KFold(len(train), n_folds=5, shuffle=True, indices=False, random_state=1337)
        results = []
        for traincv, testcv in cv:
            classifier = SGDClassifier(loss='log',n_iter=300,verbose=False,learning_rate='invscaling',eta0=0.001)
        ...
        with open(csvfile, 'w') as output:
            ...
            for i in test[myid].tolist():
                predictions += [[i,classifier.predict_proba(np.array(test[test[myid]==i][features])).tolist()[0][1]]]
...
def main():
    ...
    print "=> Stochastic Gradient Descent in action - " + str(datetime.datetime.now())
    SGD(train,test,features)
