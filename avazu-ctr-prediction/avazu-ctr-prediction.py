...
if sample:
    for classifier in classifiers:
        ...
        print 'RMSE:'
        print mean_squared_error(test.click.values, np.compress([False, True], classifier.predict_proba(np.array(test[features])), axis=1))**0.5 # RMSE

else: # Export result
    for classifier in classifiers:
        if not os.path.exists('result/'):
            os.makedirs('result/')
        predictions = np.column_stack((test['id'],np.compress([False, True], classifier.predict_proba(np.array(test[features])), axis=1)))
